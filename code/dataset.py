...
class ArrangementDataset(Dataset):
    ...
    def __getitem__(self, id):
        ...
        # each element is a list containing mel and acc matrices and chord and qpm

        qpms = [x[3] for x in data]
        qpm = qpms[0]

        mel = [x[0] for x in data]

        melx = \
            [ext_nmat_to_pr(self._combine_segments(mel[i: i + 2]))
             for i in range(0, self.num_bar, 2)]
        melx = np.array([augment_pr(pr, shift) for pr in melx])
        melxprs = np.array([pr_to_onehot_pr(pr) for pr in melx])
        melx_pr_mats = np.array([piano_roll_to_target(pr) for pr in melxprs])
        mx = np.array([target_to_3dtarget(pr_mat,
                                          max_note_count=16,
                                          max_pitch=128,
                                          min_pitch=0,
                                          pitch_pad_ind=130,
                                          pitch_sos_ind=128,
                                          pitch_eos_ind=129)
                       for pr_mat in melx_pr_mats])

        mel_segments = [ext_nmat_to_mel_pr(self._combine_segments(mel[i: i + 2]))
                        for i in range(0, self.num_bar, 2)]
        ...
        # for this task
        prs = prs #prs = prs[0]
        pr_mats = pr_mats #pr_mats = pr_mats[0]
        p_grids = p_grids #p_grids = p_grids[0]
        ...
        if self.contain_chord:
            ...
            dt_x = chord #dt_x = detrend_pianotree(p_grids, chord)  # (32, 16, 39)
            return mel_segments, prs, pr_mats, p_grids, chord, mx, qpm
        else:
...
def init_music(fn, qpm):
    ...
    bridge = data['bridge']
    piano = data['piano']
    music = PolyphonicMusic([melody, bridge, piano], beat, chord, [70, 0, 0], None, qpm)
    return music
...
def wrap_dataset(fns, ids, shift_low, shift_high, num_bar=8,
    ...
    qpm_list = []
    with open("qpm.txt", "r") as f:
        for line in f.readlines():
            qpm_list.append(float(line.strip('\n')))

    for i, ind in enumerate(ids):
        fn = fns[ind]
        music = init_music(fn, qpm_list[int(fn[-7:-4])-1])
