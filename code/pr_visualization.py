...
def plot_pianoroll(
    ...
    **kwargs
    ...
    transposed = np.where(transposed == 80, 40, transposed)
    transposed = np.where(transposed == 0, 64, transposed)
...
def to_pypr(music: "Music") -> Multitrack:
    ...
    length = 8 * 4 * music.resolution   #  num_bars = 8   beats_per_bar = 4
    ...
    for track in music.tracks:
        ...
        for note in track.notes:
            if note.velocity is not None:
                pianoroll[note.time : note.end-1, note.pitch] = note.velocity  #减一是为了区分相同音高的相连音符
            else:
                pianoroll[note.time : note.end-1, note.pitch] = 64
        track = Track(
